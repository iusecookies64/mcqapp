"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.isUserCreatedContest = void 0;
const postgres_1 = __importDefault(require("../db/postgres"));
const asyncErrorHandler_1 = require("../utils/asyncErrorHandler");
// middleware to verify if request for CRUD operations on questions for a contest is valid
exports.isUserCreatedContest = (0, asyncErrorHandler_1.asyncErrorHandler)((req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    const { contest_id } = req.query;
    const { user_id } = req;
    const checkContestAndUser = `SELECT 1 FROM contests WHERE contest_id=$1 AND created_by=$2`;
    if ((yield postgres_1.default.query(checkContestAndUser, [contest_id, user_id])).rowCount) {
        next();
    }
    else {
        res.status(403).json({
            message: "Unauthorized Request",
            status: "fail",
        });
    }
}));
